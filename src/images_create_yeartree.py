...
class KmdImagesCreateYearTree(KmdCmd.KmdCommand):
    def extendParser(self):
        ...
        self.parser.add_argument(
            'srctree', metavar='</path/to/srctree>', nargs=1, help='Path to images')
        self.parser.add_argument('yeartree', metavar='</path/to/yeartree>',
                                 nargs=1, help='Root of a date tree /aaaa/aaaammdd_hhmm.*')
                ...
                if meta:
                    ...
                    try:
                        pathdest = os.path.join(
                            self.args.yeartree[0], d.value.strftime("%Y"))
                    ...
                    KmdFiles.fileMoveRename(p, os.path.join(
                        pathdest, fdest), self.args.doit)
