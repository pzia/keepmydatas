...
class KmdImagesCheckDateTree(KmdCmd.KmdCommand):
    def extendParser(self):
        ...
        self.parser.add_argument('quarantine', metavar='</path/to/directory>', nargs=1, help='Path to move suspicious files')
    ...
    def run(self):
        ...
        for root, _, files in os.walk(self.args.datetree[0]):
            ...
            for name in files:
                ...
                if meta:
                    ...
                    try :
                        ...
                        pathdest = os.path.join(self.args.datetree[0], dexifstr)
                    except :
                    ...
                    for rk in rekp :
                        ...
                        if m != None :
                            ...
                            if dnamestr != dexifstr :
                                logging.info("Dates don't match for %s - Exif %s != Name %s", p, dexifstr, dnamestr)
                                if d.value.strftime('%y') != yy[-2:]:
                                    logging.warn("Year do not match for %s !", p)
                                d.value = d.value.strptime(dnamestr+d.value.strftime(":%H:%M"), "%Y/%m/%d:%H:%M")
                                logging.info("Setting new exif|v date to %s", d.value)
                                if self.args.doit :
                                    meta[d.key] = d
                                    meta.write()
                            continue
