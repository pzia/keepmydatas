...
class KmdImagesCheckDateTree(KmdCmd.KmdCommand):
    def extendParser(self):
        ...
        self.parser.add_argument(
            'takeout', metavar='</path/to/takeout/Photos>', nargs=1, help='Root of a TakeOut tree')

    def run(self):
        doit = self.args.doit

        for root, dirs, files in os.walk(self.args.takeout[0]):
                ...
                for m in json_data['media']:
                    if 'url' in m:
                        if 'downloaded' not in m or not m['downloaded']:
                            ...
                            logging.info("Downloading %s from Google", m['title'].encode(
                                'utf-8').replace('/', '_'))
                            if doit:
                                try:
                                    urllib.urlretrieve(m['url'], os.path.join(
                                        root, m['title'].encode('utf-8').replace('/', '_')))
                                    m['downloaded'] = True
                                except:
                                    logging.warn("Can't get %s from %s", m['title'].encode(
                                        'utf-8').replace('/', '_'), m['url'])
