...
class KmdOwncloudDedup(KmdCmd.KmdCommand):
    ...
    def extendParser(self):
        ...
        self.parser.add_argument('--moveto', metavar='</path/to/folder>', nargs=1, default=None, help='Path to move orphans (else they are renamed in place)')
        self.parser.add_argument('--forcedelete', action="store_true", help='Force removal of Owncloud conflict files')
                ...
                for i in files:
                    ...
                    if m != None :
                        ...
                        if not os.path.exists(origpath):
                            logging.debug("%s doesn't exist anymore" % origpath)
                            if self.args.moveto != None :
                                origpath = os.path.join(self.args.move_orphans, origname)
                            logging.info("Restoring file to %s", origpath)
                            if self.args.doit :
                                KmdFiles.fileMoveRename(curpath, origpath, self.args.doit)
