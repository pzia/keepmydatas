...
class KmdMboxMergeDateTree(KmdCmd.KmdCommand):
    ...
    def extendParser(self):
        ...
        self.parser.add_argument(
            'srctree', metavar='</path/to/srctree>', nargs=1, help='Path to merge')
...
#        self.parser.add_argument('duptree', metavar='</path/to/datetree>', nargs=1, help='Root of a mbox date tree for duplicates /aaaa/mm')
    ...
    def run(self):
        ...
        if os.path.exists(os.path.join(datetree, dupmboxname)):
            ...
            KmdFiles.fileMoveRename(os.path.join(
                datetree, dupmboxname), os.path.join(datetree, dupmboxname), doit)
        ...
        for root, dirs, files in os.walk(srctree):
            ...
            for name in files:
                ...
                srcbox.close()
                logging.info("Closing MBOX %s - Duplicates: %d - Kept : %d (Misc : %d)",
                             p, dupcount, keepcount, miscount)
