...
# Tag set when pictures are merged (if IPTC is in use)
exiv_changed_keywords = ["merged_kmd"]
...
exiv_ignored_properties = ["Exif.Thumbnail.JPEGInterchangeFormat", "Exif.Image.ExifTag",
                           "Exif.Photo.InteroperabilityTag", "Exif.Photo.MakerNote", "Exif.MakerNote.Offset"]
        ...
        for k in metas[pathI].iptc_keys + metas[pathI].exif_keys:
            ...
            if k in metas[mainI].iptc_keys + metas[mainI].exif_keys:
                try:
                    if metas[mainI][k].value != metas[pathI][k].value:
                        ...
                        logging.debug(
                            "%s <> %s", metas[mainI][k].value, metas[pathI][k].value)
                        if k in exiv_handlers:
                            newval = exiv_handlers[k](
                                metas[mainI][k].value, metas[pathI][k].value)
                            logging.info("Merged property %s : %s", k, newval)
                        else:
                            logging.warn("NO HANDLER for %s", k)
                except:
                    logging.warn(
                        "Coulnd't compare %s exif property for %s", k, mainI)
            else:
            ...
            if newval != None:
                try:
                    ...
                    logging.warn(
                        "Coulnd't setup %s exif property for %s", k, mainI)
    if write:
        if "Iptc.Application2.Keywords" in metas[mainI].iptc_keys:
            metas[mainI]["Iptc.Application2.Keywords"] = handler_mergeList(
                metas[mainI]["Iptc.Application2.Keywords"].value, exiv_changed_keywords)
