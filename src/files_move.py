...
class KmdFilesMove(KmdCmd.KmdCommand):
    ...
    def extendParser(self):
        ...
        self.parser.add_argument('regexp', metavar='<regexp>', nargs=1, help='Perl regexp matching files')
        self.parser.add_argument('folder', metavar='</path/to/dest>', nargs=1, help='Folder to put matching files')
        self.parser.add_argument('--mime', action="store_true", help='Filter with mime type')
            ...
            for name in files:
                ...
                if self.args.mime :
                    try : 
                        ...
                        logging.debug("Exception when check mimetype of %s" % pname)
